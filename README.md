...
> **状态（2026-09-25）**：阶段 0–7 与「新 UI」U1–U9 已完成，功能可用（接收 / 发射 / 自动程序 / 日志 / 地图 / 设置）；
> 剩余阶段 8（前台服务）与阶段 9（稳定性收尾），以及真机全流程回归。
...
| 地图页 | ✅ 已完成 | 自绘等距投影 + 大圆距离/方位 + CQ 分区；**不做**在线瓦片与海岸线底图 |
| 设置页（8 组，30+ 项） | ✅ 已完成 | DataStore 持久化；主题暗/亮、字体三档、瀑布高度即时生效 |
| 前台服务（后台持续接收） | ⏳ 阶段 8 | 当前切到系统文件选择器等导致 Activity 进后台时会 `onStop` 停止接收 |
| CI（GitHub Actions） | ⏸ 已停用 | 工作流改名为 `.github/workflows/android.yml.disabled`，只做本地构建与测试（见 `docs/BUILD.md` §5） |

**明确不做**：CAT 电台控制、Hound / Fox、FST4、接收频率自动窄带过滤、在线地图瓦片与海岸线底图、逐条呼号的精确 DXCC、日志上传（CloudLog / LoTW / eQSL）与局域网后台（后三项曾为占位，已连同代码与文档一并删除）。
