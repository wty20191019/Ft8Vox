...
class AutoProgramTest {
    ...
    fun autoCqWhenNoCandidates() {
        val d = AutoProgramSelector.decide(
            listOf(decoded("F4FSY JA1ABC -08")),
    ...
    @Test
    fun reportPriorityAnswersDirectedReport() {
        val d = AutoProgramSelector.decide(
            listOf(decoded("F4FSY JA1ABC -12", snr = -6, df = 1521)),
            program.copy(reportPriority = true),
            myCall = "F4FSY",
        )
        val t = (d as AutoDecision.AnswerReport).target
    ...
    @Test
    fun reportPriorityOffIgnoresDirectedReport() {
        val d = AutoProgramSelector.decide(
            listOf(decoded("F4FSY JA1ABC -12", snr = -6)),
    ...
    fun reportPriorityRanksReportBeforeCq() {
        ...
        assertTrue(d is AutoDecision.AnswerReport)
        assertEquals("JA1ABC", (d as AutoDecision.AnswerReport).target.call)
    }
