...
SNR 估算口径（JTDX，与 WSJT-X 同一尺度）：对每个符号 i，取**该符号实际发送音调**所在 bin 的功率 `P_sig`（含信号 + 本底噪声），
与**同一符号内、紧邻信号但排除音调区间**那一段 bin 的**算术平均** `P_noi`（只含噪声）之比 `r_i = P_sig / P_noi`；
...
噪声参考取算术均值、**不使用分布假设**，且与被测信号同符号、频带紧邻，天然抗带内强台与时间起伏。
...
> **为何不直接用「同一符号内其余 7 个音调」**：JTDX 的分析窗是 **1 个符号 + 矩形加权**，8 个音调在窗内严格正交，
> 其余音调 bin 只含噪声。本项目瀑布窗是**周期 Hann、窗长 = 1 符号周期 × freq_osr**（默认 `freq_osr=2`，约 2 个符号），
> 音调不再正交，其余音调 bin 会被**信号自身的泄漏**污染。泄漏比例虽小（约 −31 dB，即信号的 0.08%），
> 但强台的 bin 信噪比可达 +35 dB，泄漏因此达到噪声底的数倍，会把噪声底抬高：
> harness（同一份数据）实测真值 `+10 dB` 时照搬原式读到 `+2.4 dB`（**偏低约 6 dB**，且偏差随信号增强而增大），
> 改成本窗口均值后回到 `+8.6 dB`（与旧口径 `+8.3 dB` 一致）。故这里保留 JTDX 的「无分布假设的算术均值」这一核心，
> 只把噪声窗口放在本项目窗允许的位置。

经验修正两项（对应 JTDX 的「经验曲线 + 钳位」）：

1. **去偏**：噪声参考是 n 个指数分布 bin 的**平均**，`E[1/mean_n] = n/((n−1)·N)`，比真值大 `n/(n−1)` 倍，
   故逐符号乘回 `(n−1)/n`（弱台不加此项会整体偏高）。
2. **窗函数折算**：`window_db = 10·log10(RBW / 2500) + 26.5`，其中 `RBW = 1.5 · (1/符号周期) / freq_osr`
   ...
   > 常数定义在 `ftx_session.c`：`JTDX_SNR_CAL_DB 26.5`、`JTDX_SNR_MIN_DB −24`、`JTDX_SNR_MAX_DB +40`，以及噪声窗口半宽 `SNR_NOISE_WIN 16`。

**实测校核（合成信号，白噪声，同一份数据对照 `HEAD` 旧实现）**：以「真值」`SNR = P_tone/(N0·2500)` 为准，

| 真值 | −20 | −15 | −10 | −5 | 0 | +5 | +10 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 旧（p25） | −21.29 | −15.48 | −11.32 | −6.54 | −1.20 | +3.85 | +8.34 |
| 新（本窗口均值） | −21.16 | −15.49 | −11.31 | −6.02 | −1.25 | +3.82 | +8.57 |

两者相差 ≤0.5 dB，即白噪声下**口径未变**（残留约 −1.2 dB 为既有偏差）；差别只体现在真实音频的非指数噪声上。
强台场景（弱台真值 −15 dB + 1000 Hz 处带内强台）：旧「全带均值」在强台 ≥ +20 dB 时被压到 −24 dB 下限，
新实现稳定在 −18.9 ~ −19.4 dB 不变。
...
> 本项目不再复刻整带/分位噪声底，改为 JTDX 的「算术均值」以换取对噪声统计特性的鲁棒性。
