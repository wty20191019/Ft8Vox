...
其中 FT8 的 `ref` 为换算到 2500 Hz 参考带宽所需的偏置，**按每个 waterfall bin 的等效噪声带宽（RBW）实算**，不能写死常数：
`RBW = 1.5 · (1/符号周期) / freq_osr`（分析窗长 = 1 个符号周期 × `freq_osr`，周期 Hann 窗噪声等效带宽为 1.5 个 FFT bin），
故 `ref = 10·log10(RBW / 2500)`。`freq_osr=2`（FT8「标准」）时 `RBW = 4.6875 Hz → ref = −27.27 dB`，与 WSJT-X 的 `−27.0` 一致。
> 写死 `−27.0` 只在 `freq_osr=2` 成立：「快」(`freq_osr=1`) 会**偏低约 2.7 dB**、「深」(`freq_osr=4`) 会**偏高约 3.3 dB**；
> FT4 此前误用音调间隔 `1/符号周期` 而非 RBW，会偏高约 1.3 dB。结果下限为 −24 dB。
