...
> 进展（2026-09-24）：已落地 Compose 瀑布（滚动、自适应强度、点击选频，红色竖线标记）、
> 频率轴刻度、解码列表（UTC 时间 / SNR / DT / DF / 文本，点击条目即按该频率选频）、
> `SessionViewModel` 三路不可变状态（messages / waterfall / status）+ 时隙进度与奇偶周期显示。
> native 侧同时补齐解码指标与 waterfall 行流（见 `docs/JNI-CONTRACT.md` 第 4、6 节）。
> 待办：偶/奇周期背景分色、连续运行掉帧实测、真机瀑布观感调优。
