...
> - **交互微调：抽屉上滑展开 + 设置页自动程序去「二级滑动菜单」（2026-09-26）**：用户截图指着
>   操作页底部那条收起态横条（`→ 无目标　空闲/只接收　[开关]`）要求「改成上滑打开抽屉」。
>   `TxDrawer` 收起条在原有「点击 / 长按」之上加 `detectVerticalDragGestures`：松手时累计位移
>   ≤ −40dp（`DRAWER_SWIPE_DP`）即展开，**手势触发即可**（不跟手拖动，抽屉本体仍是 `ModalBottomSheet`
>   自带下滑关闭）；开关自身消费点击，拨开关不会误开抽屉。另外设置页 6.3 内嵌的 `AutoProgramPanel`
>   原本自带 `heightIn(max = 460.dp) + verticalScroll`，在设置页里形成第二层滚动 → 新增
>   `nestedScroll` 参数（默认 `true`，顶栏弹窗那份保留内层滚动），**设置页传 `false`**：面板完全展开、
>   跟随整页滚动。验收见 `REGRESSION.md` §4 与 E 组。
